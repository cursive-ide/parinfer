...
function addIndent(result, delta) {
  ...
  replaceWithinLine(result, result.lineStart, 0, origIndent, indentStr);
  result.x = newIndent;
...
function publicResult(result) {
  var final;
  var cursorX = result.cursorX !== UINT_NULL ? resultCursorX(result) : UINT_NULL;
  ...
  if (result.success) {
    ...
    final = {
      ...
      cursorX: result.partialResult ? cursorX : result.origCursorX,
